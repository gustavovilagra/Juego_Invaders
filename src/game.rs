...
use crate::{invaders::Invaders, player::Player, INVADER_HEIGHT, INVADER_WIDTH, PLAYER_SIZE, SHOT_HEIGHT, SHOT_WIDTH};
...
impl<'a> Game<'a> {
    ...
    pub fn check_game_over(&mut self, player: &mut Player, invaders: &Invaders) {
        ...
        if self.is_game_over {
            ...
            for shot in invaders.shots.iter() {
                if shot.active {
                    let shot_rect = Rect::new(shot.x, shot.y, SHOT_WIDTH as u32, SHOT_HEIGHT as u32);
    ...
    pub fn draw(&self, canvas: &mut Canvas<Window>, player: &mut Player) {
        ...
        if self.is_player_exploding && self.explosion_start.elapsed() < Duration::from_millis(500) {
            ...
            canvas.copy(&self.explosion_texture, None, Some(explosion_rect)).unwrap();

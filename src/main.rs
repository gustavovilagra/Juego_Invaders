...
fn render_text(
    ...
    y: i32,
    color: Option<Color>,
) // Añadir color como un parámetro opcional
{
    // Renderizar el texto
...
fn main() -> Result<(), String> {
    ...
    let window = video_subsystem
        .window("Game Gustavo Villagra", WIDTH, HEIGHT)
        .position_centered()
        .build()
    ...
    for _ in 0..289 {
        // Ajusta el número de estrellas según sea necesario
    ...
    canvas.clear();
    canvas
        .copy(&start_texture, None, Some(message_rect))
        .unwrap();
    canvas.present();
    ...
    fn generate_image_paths_loser() -> Vec<String> {
        (0..NUM_IMAGES_LOSER)
            .map(|i| {
                format!(
                    "resource/WhatsApp Video 2024-02-01 at 18.34.06_{:03}.png",
                    i
                )
            })
            .collect()
        ...
        'gameloop: loop {
            ...
            if game.is_game_over {
                ...
                if live > 0 {
                    ...
                    render_text(&mut canvas, &font, text, x, y, Some(font_color));
                    canvas.present();

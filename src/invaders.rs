        ...
        if self.active {
            let rect = if level == 4 {
                ...
                Rect::new(
                    self.x * 20,
                    self.y * 20,
                    INVADER_WIDTH as u32 * 12,
                    INVADER_HEIGHT as u32 * 12,
                )
            } else if level == 3 {
                Rect::new(
                    self.x * 20,
                    self.y * 20,
                    INVADER_WIDTH as u32 * 3,
                    INVADER_HEIGHT as u32 * 3,
                )
            } else {
                Rect::new(
                    self.x * 20,
                    self.y * 20,
                    INVADER_WIDTH as u32 * 2,
                    INVADER_HEIGHT as u32 * 2,
                )
            };
...
impl<'a> Invaders<'a> {
    pub fn new(
        ...
        Ok(Invaders {
            ...
            shot_timer: Instant::now()
                .checked_add(Duration::from_millis(shot_timer_duration))
                .unwrap_or(Instant::now()), // Inicializa shot_timer según el nivel
            direction: 1,
    ...
    pub fn update(&mut self) {
        ...
        if self.last_update.elapsed() > self.move_timer {
            ...
            if self.level == 3 {
                ...
                if self.direction == 1
                    && (max_x + INVADER_WIDTH * size_factor3 / 20) >= WIDTH as i32 / 20 - 1
                {
                    self.direction = -1;
                    move_down = true;
                ...
                // Cambiar la dirección y decidir si mover hacia abajo
                if self.direction == 1
                    && (max_x + INVADER_WIDTH * size_factor4 / 20) >= WIDTH as i32 / 20 - 1
                {
                    self.direction = -1;
                    //move_down = true;
            ...
            if self.level == 4 {
                ...
                if move_down {
                    ...
                    self.move_timer =
                        Duration::from_millis((200f32 / self.speed_increase_factor) as u64);
                }
            } else if self.level == 3 {
                if move_down {
                    self.speed_increase_factor *= 1.1;
                    self.move_timer =
                        Duration::from_millis((700f32 / self.speed_increase_factor) as u64);
                }
            } else {
                if move_down {
                    ...
                    self.speed_increase_factor *= 1.1;
                    self.move_timer =
                        Duration::from_millis((500f32 / self.speed_increase_factor) as u64);
                }
            }
    ...
    pub fn check_player_hits(&mut self, player_shots: &mut Vec<Shot>) -> bool {
        ...
        for shot in player_shots.iter_mut() {
            ...
            for invader in self.army.iter_mut() {
                ...
                if self.level == 4
                    && invader.active
                    && shot_collides_with_invader(shot, invader, self.level)
                {
                    invader.hit_count += 1;
                    shot.active = false;
...
fn shot_collides_with_invader(shot: &Shot, invader: &Invader, level: u32) -> bool {
    ...
    let invader_rect = if level == 4 {
        Rect::new(
            ...
            invader.y * 20,
            INVADER_WIDTH as u32 * 12,
            INVADER_HEIGHT as u32 * 9,
        ) // Tamaño más grande para el invasor del nivel 4
    } else if level == 3 {
        Rect::new(
            invader.x * 20,
            invader.y * 20,
